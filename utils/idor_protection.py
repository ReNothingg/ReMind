    ...
    def decorator(view_func):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                return make_error("Invalid session", status=401, code="invalid_session")
            ...
            if not resource_id:
                return make_error(
                    "Resource ID not provided", status=400, code="missing_resource_id"
                )
            ...
            if resource_type == "chat":
                chat = UserChatHistory.query.filter_by(
                    user_id=user_id, session_id=str(resource_id)
                ).first()
                ...
                share = ChatShare.query.filter_by(session_id=str(resource_id)).first()
...
def verify_resource_access(user_id, resource_type, resource_id):
    ...
    if resource_type == "chat":
        chat = UserChatHistory.query.filter_by(user_id=user_id, session_id=str(resource_id)).first()
        return chat is not None

    elif resource_type == "chat_share":
        share = ChatShare.query.filter_by(user_id=user_id, session_id=str(resource_id)).first()
