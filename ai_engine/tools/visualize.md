...
The runtime provides `window.remind` and the compatible alias `window.openai` with these read-only fields:

- `appearance.theme`: `light` or `dark`.
- `appearance.locale`, `appearance.direction`, and `appearance.accentColor`.
- `appearance.compactMode`, `appearance.highContrast`, and `appearance.reducedMotion`.
- `appearance.fontFamily` and `appearance.fontSize`.
- `tokens`: resolved values for every CSS token listed below.
- `capabilities`: `localInteraction`, `followUpMessage`, `networkRequests`, and `persistentStorage`.
- `sendFollowUpMessage({ prompt, title })`: asks the user to confirm a new ReMind chat message. Call it only from an explicit user action and include the selected values in the prompt.
- `renderMath(expression, { displayMode })`: asynchronously returns safe KaTeX HTML for a formula created or changed by JavaScript.
- `renderMathElement(element, expression, { displayMode })`: asynchronously renders a dynamic formula into an element.

The fragment receives these ReMind CSS tokens: `--background`, `--foreground`, `--card`, `--card-foreground`, `--popover`, `--popover-foreground`, `--primary`, `--primary-foreground`, `--secondary`, `--secondary-foreground`, `--muted`, `--muted-foreground`, `--accent`, `--accent-foreground`, `--destructive`, `--border`, `--input`, `--ring`, `--blue`, `--orange`, `--green`, `--red`, `--purple`, `--yellow`, `--viz-series-1` through `--viz-series-6`, `--font-family`, `--font-size-base`, and `--radius-sm` through `--radius-lg`. Use these tokens for every color, font, border, and radius. Never hardcode a light or dark palette. Never use gradients or shadows.

The runtime also supplies the native ReMind utility classes `.card`, `.viz-grid`, `.viz-row`, `.viz-controls`, `.viz-stat-value`, `.viz-badge`, `.btn`, `.btn-primary`, `.btn-ghost`, `.btn-block`, `.form-label`, `.form-control`, `.form-select`, `.form-range`, `.form-check`, `.table`, `.table-responsive`, `.table-sm`, `.text-small`, `.text-muted`, `.text-destructive`, `.text-end`, `.text-center`, `.text-nowrap`, and `.sr-only`. Compatibility layout helpers are `.d-flex`, `.justify-content-between`, `.align-items-center`, `.p-3`, `.mb-3`, `.m-0`, and `.h5`. Prefer these classes over recreating control or card styling. Bootstrap, Tailwind, and arbitrary utility classes are not loaded; never use a class outside this documented set unless you define it in the fragment's scoped `<style>`.

Write every visible label in the user's response language. If JavaScript changes a label, every state of that label must use the same language; do not switch between localized text and English labels such as Play or Stop.

Static LaTeX in ordinary text is rendered automatically. Use `$...$` or `\\(...\\)` inline and `$$...$$` or `\\[...\\]` for display math. Delimiters inside `script`, `style`, `textarea`, `pre`, `code`, and SVG are left untouched. For a formula that changes after interaction, use `await window.remind.renderMathElement(target, expression, { displayMode: false })`; never implement a formula renderer yourself.

### Runtime and safety

The fragment runs in a sandbox without access to ReMind cookies, origin, local storage, session storage, the parent DOM, camera, microphone, location, popups, forms, or navigation. `fetch`, XHR, WebSocket, EventSource, and all other connections are blocked. Inline CSS and JavaScript are supported. Version-pinned scripts may load only from `cdnjs.cloudflare.com`, `esm.sh`, `cdn.jsdelivr.net`, or `unpkg.com`; fonts may load only from Google Fonts, Bunny Fonts, or jsDelivr (used by the built-in KaTeX renderer). Never depend on another origin or an API request.

Use semantic HTML and native keyboard-accessible controls. Never add `tabindex`; preserve browser focus styles. Avoid fixed outer widths, viewport-height layouts, horizontal page overflow, and `position: fixed`. Size SVG or canvas from its actual container and redraw with `ResizeObserver`. Keep visible text at least 11px, pair color with labels or shapes, add accessible names/descriptions to charts, and honor reduced motion. Ensure every queried element exists, JavaScript has no undefined identifiers, and the primary interaction updates the view.

SVG and canvas APIs cannot resolve CSS variables directly. Resolve colors first with `window.remind.tokens['--foreground']`, `window.remind.tokens['--border']`, and `window.remind.tokens['--viz-series-1']` through `--viz-series-6`; redraw when appearance changes. Never use literal black or white for a spring, mass, axis, label, or other mark.
...
- Use a tooltip unless it would distract from a simple, directly labeled chart.
  ...
  and units; the sandbox handles themed positioning and keyboard focus.
